...
async function handleFetch(req,env){
  ...
  try{
    ...
    if(u.pathname==='/predictor/pools'&&req.method==='POST'){
      const body=await req.json(); const action=String(body.action||'join'); const nickname=String(body.nickname||'Speler').trim().slice(0,30); const deviceId=String(body.deviceId||'').trim().slice(0,120); if(!deviceId)return json({message:'Geen toestel-ID nie.'},400);
      if(action==='create'){
        ...
        if(!code)return json({message:'Pool-kode kon nie geskep word nie.'},500); const name=String(body.name||`${nickname} se LEKKER Rugby Pool`).trim().slice(0,60);
    ...
    if(u.pathname==='/news/live'&&req.method==='GET'){
      const q=encodeURIComponent('rugby Springboks All Blacks England Ireland France Australia Argentina Fiji Wales Scotland Samoa');
      const url=`https://news.google.com/rss/search?q=${q}&hl=en&gl=ZA&ceid=ZA:en`;
      try{
        const r=await fetch(url,{headers:{'user-agent':'LEKKER-RUGBY/1.0'},cf:{cacheTtl:300,cacheEverything:true}});
        if(!r.ok)throw new Error('News feed unavailable');
        const xml=await r.text();
        const items=[]; const blocks=xml.match(/<item>[\s\S]*?<\/item>/gi)||[];
        const clean=v=>String(v||'').replace(/<!\[CDATA\[|\]\]>/g,'').replace(/<[^>]+>/g,'').trim();
        for(const b of blocks.slice(0,12)){
          ...
          items.push(['📰',title,source?`${source} • ${pub}`:pub,link,'']);
        }
        return json({provider:'Google News RSS',updatedAt:new Date().toISOString(),items});
      }catch(e){return json({items:[],message:'Live news feed unavailable.'},502)}
    }
