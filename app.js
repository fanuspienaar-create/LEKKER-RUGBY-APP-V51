...
function screen(id){$$('.screen').forEach(x=>x.classList.toggle('active',x.id===id));$$('.bottom-nav button').forEach(x=>x.classList.toggle('active',x.dataset.screen===id));window.scrollTo({top:0,behavior:'smooth'});if(id==='rankings')renderRankings();if(id==='vote'){renderPositions();loadHistoricalPlayerDirectory().then(()=>renderPositions()).catch(()=>{});}if(id==='players'){renderPlayers();loadHistoricalPlayerDirectory().catch(()=>{});}if(id==='admin')renderAdmin();if(id==='stats'){startLiveStats()}if(id==='teamstats')renderTeamStats();if(id==='news'){renderNews();loadLiveNews().catch(()=>{});}if(id==='predictor')renderPredictor();}
function getCandidates(pos){
  const source=Array.isArray(directoryPlayers)&&directoryPlayers.length?directoryPlayers:players;
  return source.filter(p=>allowedPlayerCountry(p)&&playerPositionCodes(p).has(String(pos.n))).sort((a,b)=>a.name.localeCompare(b.name));
}
...
const PLAYER_POSITION_OVERRIDES={
  "Cheslin Kolbe":["11","14","15"],"Damian Willemse":["10","11","12","13","15"],"Jesse Kriel":["12","13","15"],"André Esterhuizen":["12","13"],"Andre Esterhuizen":["12","13"],"Sacha Feinberg-Mngomezulu":["10","12"],"Manie Libbok":["10","15"],"Handre Pollard":["10","12"],"Handré Pollard":["10","12"],"Canan Moodie":["11","13"],"Aphelele Fassi":["11","15"],"Kurt-Lee Arendse":["11","14","15"],"Ethan Hooker":["11","13"],"Edwill van der Merwe":["11","14"],"Lukhanyo Am":["12","13"],"Francois Steyn":["10","12","13","15"],"Jean de Villiers":["11","12","13"],"Bryan Habana":["11","14"],"Willie le Roux":["11","14","15"],"Percy Montgomery":["10","11","15"],"Faf de Klerk":["9","10"],"Ruan Pienaar":["9","10"],"Cobus Reinach":["9","11"],"Joost van der Westhuizen":["9","10"],"Fourie du Preez":["9","10"],"Naas Botha":["10","12"],"Morne Steyn":["9","10"],"Jannie de Beer":["9","10"],"Pieter-Steph du Toit":["5","6","7"],"Siya Kolisi":["6","7","8"],"Jasper Wiese":["6","7","8"],"Duane Vermeulen":["6","7","8"],"Kwagga Smith":["6","7","8"],"Schalk Burger":["6","7","8"],"Juan Smith":["6","7"],"Victor Matfield":["4","5"],"Bakkies Botha":["4","5"],"Eben Etzebeth":["4","5"],"Lood de Jager":["4","5"],"RG Snyman":["4","5"],"Franco Mostert":["4","5"],"Ox Nche":["1","3"],"Thomas du Toit":["1","3"],"Wilco Louw":["1","3"],"Trevor Nyakane":["1","3"],"Frans Malherbe":["1","3"],"Bismarck du Plessis":["2","8"],"John Smit":["2","3"],"Malcolm Marx":["2","8"],"Deon Fourie":["2","6"],"Johan Grobbelaar":["2","8"],"Francois Pienaar":["6","7","8"],"Bob Skinstad":["6","7","8"],"Gary Botha":["2","8"],"Ben-Jason Dixon":["4","5","6","7"],"Elrigh Louw":["6","7","8"],"Marco van Staden":["6","7"],"Cameron Hanekom":["6","7","8"],"Vincent Tshituka":["6","7","8"],"Cobus Wiese":["4","5","6"],"Ruan Nortje":["4","5"],"Sacha Feinberg-Mngomezulu":["10","12"]
};
const CURRENT_BOK_SCORING={"Kurt-Lee Arendse":{points:135,tries:26},"Jesse Kriel":{points:125,tries:25},"Malcolm Marx":{points:145,tries:29}};
const CURRENT_BOK_CAPS={
  "Deon Fourie":15,"Johan Grobbelaar":10,"Malcolm Marx":92,"Jan-Hendrik Wessels":14,"Thomas du Toit":36,"Wilco Louw":34,"Ox Nche":51,"Zachary Porthen":7,"Carlü Sadie":1,"Gerhard Steenekamp":22,"Boan Venter":10,
  "Lood de Jager":76,"Eben Etzebeth":145,"Franco Mostert":84,"Ruan Nortjé":23,"Paul de Villiers":4,"Ben-Jason Dixon":11,"Pieter-Steph du Toit":101,"Cameron Hanekom":5,"Siya Kolisi":106,"Elrigh Louw":15,"Vincent Tshituka":2,"Marco van Staden":37,"Cobus Wiese":6,"Jasper Wiese":48,
  "Herschel Jantjies":25,"Cobus Reinach":56,"Morné van den Berg":9,"Grant Williams":31,"Sacha Feinberg-Mngomezulu":23,"Manie Libbok":33,"Vusi Moyo":1,"Handré Pollard":87,"Handre Pollard":87,
  "Damian de Allende":102,"André Esterhuizen":34,"Andre Esterhuizen":34,"Jesse Kriel":93,"Kurt-Lee Arendse":35,"Ethan Hooker":12,"Cheslin Kolbe":52,"Canan Moodie":25,"Edwill van der Merwe":7,"Aphelele Fassi":18,"Quan Horn":2,"Damian Willemse":54
};
function playerPositionCodes(p){
  const set=new Set();
  if(p?.pos)set.add(String(p.pos));
  const ov=PLAYER_POSITION_OVERRIDES[p?.name];(ov||[]).forEach(x=>set.add(String(x)));
  if(Array.isArray(p?.positions))p.positions.forEach(x=>set.add(String(x)));
  return set;
}
...
function playerStatsData(name){
  ...
  if(d){
    if(CURRENT_BOK_CAPS[name]) return {...d,caps:CURRENT_BOK_CAPS[name],to:2026,...(CURRENT_BOK_SCORING[name]||{})};
    return d;
  }
...
async function loadHistoricalPlayerDirectory(){
  ...
  const cacheName='lekker-rugby-player-data-v2';
  let cache=null; try{cache=await caches.open(cacheName)}catch(e){}
  const getText=async(url)=>{try{let r=cache?await cache.match(url):null;if(!r){r=await fetch(url,{cache:'force-cache'});if(r.ok&&cache)cache.put(url,r.clone()).catch(()=>{});}return r&&r.ok?r.text():'';}catch(e){return ''}};
  await Promise.all(HISTORICAL_PLAYER_SOURCES.map(async ([country,flag,file])=>{
    const text=await getText(HISTORICAL_PLAYER_BASE+file);if(!text)return;
    ...
    for(const line of lines.slice(1)){
      ...
      const player={name,country:flag,pos:positionNumber(row[pi])};
      ...
      const currentCaps=CURRENT_BOK_CAPS[name];
      historicalPlayerStats[name]={caps:currentCaps||matches,points:(CURRENT_BOK_SCORING[name]?.points??points),tries:(CURRENT_BOK_SCORING[name]?.tries??tries),from:year,to:currentCaps?2026:year};
...
let liveNewsItems=[];
let liveNewsUpdatedAt=0;
async function loadLiveNews(){
  if(!API_URL)return false;
  try{const r=await fetch(API_URL.replace(/\/$/,'')+'/news/live',{cache:'no-store'});if(!r.ok)return false;const d=await r.json();if(Array.isArray(d.items)&&d.items.length){liveNewsItems=d.items;liveNewsUpdatedAt=Date.now();renderNews();return true;}}catch(e){} return false;
}

function renderNews(){
  const el=document.getElementById('newsList');if(!el)return;
  const items=liveNewsItems.length?liveNewsItems:LEKKER_NEWS;
...
function startLiveStats(){clearInterval(liveTimer);loadLiveStats();liveTimer=setInterval(()=>{loadLiveStats()},300000)}
let liveAppTimer=null;
function startLiveApp(){clearInterval(liveAppTimer);loadLiveStats();loadLiveNews().catch(()=>{});liveAppTimer=setInterval(()=>{loadLiveStats();loadLiveNews().catch(()=>{})},300000)}
...
loadLocalPlayers();loadVoteCounts().then(()=>renderPositions());progress();renderRankings();renderPlayers();renderAdmin();renderTeamStats();renderNews();startLiveApp();
